...
normalize.Cel.container.qspline <- function(listcel, ...) {
  x <- data.matrix(data.frame(lapply(listcel@x, function(y) c(intensity(y)))))
...
normalize.qspline <- function(x,
  ...
  if(is.null(target)) target <- exp(apply(log(x), 1, mean))
  x.n <- dim(x)[1]
  m   <- dim(x)[2]
  
  if(is.null(samples)) samples <- max(round(x.n/1000), 100)
  else if(samples<1)   samples <- round(samples*x.n)
  p <- 1:samples/samples
  p <- p[which(p<=p.max)]
  p <- p[which(p>=p.min)]
  samples <- length(p)
  ...
  k <- fit.iters
  if(na.rm==TRUE) y.n <- length(which(!is.na(target)))
  ...
  for (i in 1:m) {
    ...
    if(na.rm==TRUE)  x.valid <- which(!is.na(x[,i])) 
    else             x.valid <- 1:x.n
    x.n <- length(x.valid)
