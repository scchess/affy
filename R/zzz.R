...
.initBackgroundCorrect <- function(where) {
...
.First.lib <- function(libname, pkgname, where) {
  ...
  ## DEBUG flag
  assign("debug.affy123", F, envir=where)
  ...
  .initNormalize(where)
  .initExpression(where)
  .initBackgroundCorrect(where)
