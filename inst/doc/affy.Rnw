...
\verb+affy+ is part of the Bioconductor project. It is meant to be 
...
As mentionned earlier, the aim is flexibility rather than speed. The
path going through the Plob objects  
can sometimes be faster
% really true ?
while the path going through PPSet.container is more flexible and can
integrate easily user defined 
functions.
