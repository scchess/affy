...
%\VignetteIndexEntry{affy2 primer}
...
\author{Laurent Gautier and Rafael Irizarry}
\begin{document}
\title{Textual description of affy (version 2.0)}

\maketitle
\tableofcontents
\section*{Introduction}
The \verb+affy+ package is part of the Bioconductor\footnote{\url{http://www.bioconductor.org/}} project. It is 
...
\item[mismatch] the probes having one base mismatch
  with the target sequence intended to account for nonspecific binding. 
...
\item[probe pair set] several probe related to a common {\it affyID}. 
...
Section 2 described the main differences between version 1.0 and
version 2.0. Section 3 describes a quick way of getting started and
getting expression measures, more too come.
 
\section{Changes from Version 1.0} 
{\bf What's new?}
\begin{itemize}
\item Faster reading functions (\verb+?read.affybatch+)
\item Widgets for reading phenotypic, MIAME information and choices of
...
\item MAS 5.0 available (\verb+?expresso+).
\item RNA degradation assessment. (\verb+?AffyRNAdeg+)
\end{itemize}

The main difference between Version 1.0 and version 2.0 is that the
...
Simply download as many of these {\it cdf enviroments} as you need and install
them. The affy package will now where to look.
...
environments. See Section \ref{sec:probesloc} to do this.

The new version  provides a unified approach to working with probe level
data. The main object the user will manipulate is called {\it AffyBatch}
...
The classes {\it Cdf}, {\it ProbeSet} are briefly described, but the
novice user will not have to know much about them.
...
And download the cdf environment you need
\url{http://www.bioconductor.org/data/cdfenvs/cdfenvs.html} (except if
you are using the HGU95Av2 or HGU133A chip).

\subsection{Quick start}
If all you want is to go from probe level data ({\it Cel} files) to
expression measures here are some quick ways of doing this.

The quickest way of reading in data is the following:
Create a directory, move all the relevant
{\it CEL} files to that directory. Start R in that directory. If using
Microsoft Windows make sure you working directory contains the {\it
Cel} files (the function {\verb+getwd+, \verb+setwd+, and
\verb+list.celfiles+} are useful for this). 
Load the library and then type: 

\begin{Sinput}
R> Data <-  ReadAffy() ##read data in working directory
R> eset <- rma(Data)
\end{Sinput}
The \verb+rma+ function was written by Ben Bolstad in C for speed and
efficiency. It uses the expression measure described in \cite{iriz:etal:2003}.

For other popular methods use \verb+expresso+ instead of \verb+rma+. For example:
\begin{Sinput}
R> eset <- expresso(Data, normalize.method="qspline", bg.method="rma", 
                    summary.method="liwong")
\end{Sinput}

For creating your own expression measures you can use \verb+express+
\begin{Sinput}
R> Data <-  ReadAffy() ##read data in working directory
R> eset <- expres(Data, summary.method=function(x) apply(x,2,median))
\end{Sinput}

The variable \verb+eset+ is an object of class \verb+exprSet+
...
The function \verb+ReadAffy+ described above is quite flexible 
and lets you specify the filenames,
and to enter phenotypic and MIAME information. 
You can enter them using files (see the help
...
of the {\it tkWidgets} package (function written by Jianhua (John) Zhang).}
\end{center}
\end{figure}
...
to enter phenotypic or covariate information. See Figure \ref{fig:widget.filechooser}.
...
This widget is part
of the {\it tkWidgets} package (functions written by Majnu John.}
\end{center}
\end{figure}


Finally the a widget is presented for the user to enter MIAME information.
See in Figure\ref{fig:tkMIAME}.
...
of the {\it tkWidgets} package (function written by Majnu John).}
\end{center}
\end{figure}
...
read it from flat files or to enter it by hand.

The function \verb+ReadAffy+ is a wrapper for the function
\verb+read.affybatch+, \verb+tkSampleNames+, \verb+read.phenoData+, 
and \verb+read.MIAME+.  
The function \verb+read.affybatch+ has some nice feature that make it quite
flexible. 
...
are compress and let you read them while they remain compressed. 
...
expression values. 
Typically this is achieved by going through a sequence of steps: 
\begin{enumerate}
...
and, in some cases, on standard error for this summary.
\end{enumerate}
We detail what we believe is a good way to proceed below.

As mentioned the function \verb+expresso+ provides many options.
\begin{Sinput}
R> eset <- expresso(affybatch, normalize.method="qspline", bg.method="rma", summary.method="liwong")
\end{Sinput}

This will store expression values, in the object \verb+eset+, as an 
...
The function \verb+rma+ computes the RMA expression measure. Because
its hard coded in C it is quite fast and efficient. The function
\verb+express+ lets the user define their own functions as send them
as arguments.
 
The function {\it expresso} performs the steps background correction, normalization,
probe specific correction, and summary value computation. 
This function too has several parameters to facilitate your work. We
now show this using an data included in the package.

\begin{description}
 \item[normalize.method]. The normalization method to use. The
   available methods can be queried by using 
{\it normalize.methods}.
<<>>=
data(affybatch.example) ##data included in the package for examples
normalize.methods(affybatch.example)
@ 

\item[background.method]. The background correction method to use.
<<>>=
bgcorrect.methods
@ 

\item[summary.method]. The summary method to use.
<<>>=
express.summary.stat.methods
@

\item[widget]
The widget will let you select missing parameters (like the
normalization method, the background correction 
method or the summary method).
The figure~\ref{fig:widget.normalizechooser} presents the widget for the
selection of a different methods.
...
Options for the package can be configured (see Section~\ref{sec:configure.options}).
...
\url{http://qolotus02.genelogic.com/datasets.nsf/} and described in \cite{iriz:etal:2003}. The help file obtained via \verb+?Dilution+ describes the data set.
...
This will create the \verb+Dilution+ object of class \verb+AffyBatch+. 
\verb+print+ (or \verb+show+) will show some relevant information.
These objects 
are meant to represent data from one 
experiment. The \verb+AffyBatch+ class combines the
...
same RNA.  This information is part of the \verb+AffyBatch+ and can be
...
accessed with the \verb+pm+, \verb+mm+, and \verb+probeNames+ methods. 
...
$i$th entry of the vector returned by \verb+probeNames+.

<<>>=
Index <- c(1,2,3,100,1000,2000) ##6 arbitrary probes
pm(Dilution)[Index,]
mm(Dilution)[Index,]
...
names and the row names are the {\it affyID} together with the probe
...
The \verb+pm+ and \verb+mm+ functions can be used to extract specific
probe set intensities.
<<>>=
gn <- geneNames(Dilution)
pm(Dilution,gn[1000])
@ 
Notice that the method \verb+geneNames+ extracts the unique {\it
affyID}s. Also notice that the 1000th probeset is different from the
1000th probe! The 1000th probe is not part of the the 1000th probe set.

\verb+boxplot()+, \verb+hist()+, and
\verb+image+ 
are examples of useful methods available for objects of this class.   
For example, Figure \ref{f3} shows histograms of $PM$ intensities
...
\end{center}
\end{figure}
Notice in Figure \ref{f3} that we see two small ``bumps'' near the largest
value. This is evidence of saturation.

As seen in the previous example, the sub-setting method \verb+[+ can be used to extract specific
arrays. {\bf NOTE: Sub-setting is different in version 2.0. One can no
...
The method \verb+boxplot+ can be used to show $PM$, $MM$ or both intensities.
\begin{figure}[htbp]
\begin{center}
<<fig=TRUE>>=
par(mfrow=c(1,1))
...
@
The \verb+boxplot+ method for the \verb+AffyBatch+ class, shown in
...
file). The default is quantile normalization (see
\cite{bols:etal:2002}).  Other methods are
<<>>=
...
\section{ProbeSet data}
The \verb+ProbeSet+ class holds the information of all
the probes related to a {\it affyID}. The components are \verb+pm+ and
\verb+mm+.
...
the CDF file. However, the user can decide what each probe set is. The 
argument \verb+locations+ lets the user decide the row numbers in the 
\verb+intensity+ that define a probe set. For example, we are
interested in redefining the \verb+1000_at+ and \verb+1001_at+ probe set, we
could write
<<>>=
...
example by considering only probes common to both arrays. More on this
in Section \ref{probesloc}.

An example spike in data set is included in the package in the form of
a Probe Set. The help file describes the data set.
...
matplot(concentrations,pms,log="xy",main="PM")
lines(concentrations[1,],apply(pms,2,mean),lwd=3)
##so do mms
matplot(concentrations,mms,log="xy",main="MM")
lines(concentrations[1,],apply(mms,2,mean),lwd=3)
@ 
\caption{\label{f5.3}PM and MM intensities plotted agains SpikeIn concentration}
\end{center}
\end{figure}
...
%e <- express(Dilution)
%@
%returns expression measures in an object of class {\tt exprSet}
...
%data. There is a \verb+write+ method which will write expression
%tables to a file for users who wish to use other packages for analyses.
%
...
%
%The class  \verb+AffyBatch+ combines the
...
%Objects of class \verb+AffyBatch+  have a different 
...
%\begin{description}
%  \item[generateExprSet] Compute expression values (for all the {\it affyIDs} on the chip, or for a specified
%  subset of them) and return them in an \verb+exprSet+ object.
...
%is straight forward. Two lines give us a naive version\footnote{here we do not consider trim or outliers removal}
%of Affymetrix's average difference measure.
%
%<<>>=
...
%Currently XYZ can only be \verb+Cel.container+ or \verb+AffyBatch+\footnote{A general scheme for normalization of arrays, whether
...
%\label{s3.2}
%
%Things described in this section could change slightly in the near future.
%If you develop methods for the package, you are warmly encouraged to look at
%it again when upgrading this package.
%
%As seen in Section \ref{s1.5} the function \verb+express+ creates
%\verb+exprSet+ from \verb+AffyBatch+ objects. The function \verb+express+
%is quite flexible.
%
...
%performed. If a function is sent, it must receive two matrices. The
%first matrix will contain the $PM$s while the second matrix will contain
%the corresponding $MM$s. The
...
%\end{verbatim}
%
%Once background corrected probe intensities are available we compute expression
%measures via some summary statistic. The probe data will be a matrix,
%say \verb+mat+ of
...
%A way to include new approaches is to go through the \verb+Cel+ and \verb+PPSet+
%objects, or more precisely through \verb+Cel.container+ and \verb+PPSset.container+ objects.
%
%As described above, a \verb+PPSet.container+ is used to bundle several
%\verb+PPSet+  
%objects and it                                                                      
%used here to collect the {\it PPSet} for a given gene in different 
%hybridizations (i.e. on different arrays).
%The method \verb+express.summary.stat+ is defined for objects of class
%\verb+PPSet.container+. Its signature is
%\verb+(x="PPSet.container", method="character", bg.correct="character")+. 
%The parameter \verb+method+ will indicate how to compute the expression
%value, and the parameter \verb+bg.correct+ how to perform the {\it background correction}.
%
%Methods that generate expression values accept a matrix of probe intensities (one row per probe in the set,
% one column per experiment (or oligonucleotide array).
%They as first parameter a matrix
% of $PM$s values and as second parameter a matrix of $MM$s values. 
%The method can then accept other optional parameters, but default must
%be defined. The name of the method must be of the form \verb+generateExprVal.method.xyz+, where
%\verb+xyz+ in the name of the method. Then the methods nicknames should
%be registered in \verb+express.summary.stat.methods+. 
...
%              
%  probes.med <- apply(probes, 2, median, na.rm=TRUE)
%  probes.mad <- apply(probes, 2, mad, na.rm=TRUE)
%  ## replace the 'outliers' by the median value                  
%  probes[abs(probes - probes.med) > 2 * probes.mad)] <- probes.med                                                                                                     
%  r <- apply(probes, 1, mean, na.rm=TRUE)                                                                                                       
%  return(r)                                                                             }                                                                         
%## register the method
%express.summary.stats..methods <- c(generateExprSet.methods, "outlieraverage")
%\end{verbatim}                                                                                        
%
%The background correction methods differ slightly from the ones used with \verb+AffyBatch+.
%They must have as first parameter a matrix of $PM$ values and as a second parameter
% a matrix of the corresponding $MM$s. These are the values for the probes relating to
%one {\it affyID} across different arrays ({\it i.e.} the values contained in a
%\verb+PPSet.container+)\footnote{{\it AffyBatch} work on all the $PM$s and $MM$s on the array
%in one call (note: So looping through all the affyIDs will be slower, but offers more 
%flexibility... but we may unify all that in the future...)}. The \verb+bg.correct+
% methods available in the package are stored in the variable \verb+bg.correct.methods+.
%
%When calling the method \verb+generateExpSet+ on an object of class \verb+Cel.container+,
%the program will loop through all the {\it affyIDs} on the array (or through a specified subset,
%if any), extracting the \verb+PPSet.container+ for each of them and performing background
%correction followed by the computation of the expression value that summarizes the probes intensities.
%An object of class \verb+exprSet+\footnote{defined in the package Biobase} is returned.
%

\section{Probes location}
\label{sec:probesloc}
On oligonucleotide arrays, several probes give a measure of the expression
level for each gene. To process the data, one must know where are
located the probes related to a particular gene. This is done by
storing information of an {\it R} environment. The slot of {\it AffyBatch}
{\it cdfName} gives the name of the related cdf information. This
corresponds to the name of an environment.


Environments will be available as packages to download shortly. In
the meanwhile, you will have to call the function
'createLocationsEnvironment' with the name of the CDF file
corresponding to your CEL files.

Example:
\begin{Sinput}
R> createLocationsEnvironment("affycdf/Hu6800.CDF")
\end{Sinput}
...
Package-wide options can be configured. We will show how to configure the options with a first example:
the compression of files. If you are always compressing your CEL files, you might find annoying to specify
it each you read them. I can be specified once for all in the options.
<<>>=
opt <- getOption("BioC")
affy.opt <- opt$affy
names(affy.opt)
...
The second example explains how works the locations of probes on an array. 
<<>>=
opt <- getOption("BioC")
affy.opt <- opt$affy
print(affy.opt$probesloc)
@
The option {\it probesloc} is a list. Each element of the list is it self a list with two elements {\it what}
and {\it where}. When looking for the informations about the locations of the probes on the array, the elements
in the list will be looked at sequencially. The first one leading to the informations is used (an error
message is returned if none permits to find the information). The element {\it what} can be one of
{\it package}, {\it environment}
