...
\begin{description}
  ...
  \item[normalize.methods] Returns the known normalization methods for the \verb+Cel+.
...
\section{Overview through steps in the data processing/data analysis}
...
%   \label{fig:image}
%   \begin{center}
%     \includegraphics[scale=.7,angle=0]{image1}
%   \end{center}
%   \caption{}
%\end{figure}

 \subsection{Normalization}
\label{sec:normzalize}
...
comparable. The different approaches will only be outlined here. The reader will refer to XXX (ref!) for more details.
...
XXX
%>>>L  Rafael.... could you have one for the Plobs ?
