...
Methods that generate expression values accept a matrix of probe intensities (one row per probe in the set, one column per experiment (or oligonucleotide array).
...
sets from the different hybridizations are processed.                                   Probe values that are more than  2 times away from the median absolute
...
one {\it affyID} across different arrays ({\it i.e.} the values contained in a \verb+PPSet.container+)\footnote{{\it Plob} work on all the $PM$s and $MM$s on the array in one acall (note: So looping through all the affyIDs will be slower, but offers more 
