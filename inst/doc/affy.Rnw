...
\title{NOTES ON THE AFFY (version 1.0) PACKAGE}

% alphabetical order
% :-)
\author{Laurent Gautier\footnote{<laurent@cbs.dtu.dk>, Center for Biological Sequencing, DTU, Denmark},
	 Rafael A. Irizarry\footnote{<rafa@jhu.edu>, Department of Biostatistics (JHU), USA}}
...
The package requires R-1.5.0 (or eventually greater) and relies on the presence of other packages or on the presence of external libraries for some of its
funcionalities. More specifically:
\begin{enumerate} 
 \item The library {\it zlib} is required to read compressed data files with the
functions \verb+read.cdffile+ and \verb+read.celfile+
 ...
 \item The R package \verb+tkWidgets+ is required for the user interface \emph{widgets}.
\end{enumerate}
...
The package has demo script showing some of the functionalities. Enter \verb+demo(affy.tour)+ after having loaded the library (with the command \verb+library(affy)+.
...
sd & standard deviation for the \emph{spot}.\\
masks & \\
outliers & \\
...
\begin{tabular}{|p{3.5cm}|p{8cm}|}
%\label{table:Cel.container}
\hline
\multicolumn{2}{|l|}{\textbf{Class Cel.container - methods}}\\
\hline
normalize & return normalized object.\\
normalize.methods & list the know normalization methods.\\
show & Outputs few general facts about the object on the console.\\
\hline
\end{tabular} 
...
\hline
show & Outputs few general facts about the object on the console.\\
barplot & Barplot of the probes data.\\
plot & Line plot of the probes data.\\
\hline
\end{tabular}

\begin{tabular}{|p{3.5cm}|p{8cm}|}
%\label{table:PPSet.container}
\hline
...
be preoccupied (in a first time) by the intermediate steps.
They reflect recent work about the processing of probe level data
%>>>LG: Hey, Rafael I intend to quote your paper here, update on the refs please. Has it been submitted
 and provide what we believe being a good way to obtain expression values.
...
E <- express(Data)
\end{verbatim}
%>>>LG: Are the widgets ready ? would be better here...
...
\end{verbatim}
or if the \verb+tkWidgets+ library is available, the {\it CEL} file can be picked with the mouse by using
%>>>LG EXAMPLE !!!
XXX
...
Different approaches to normalize array data have already been suggested, but new methods are very likely be developped too, so we tried to make the addition of new normalization methods straightforward.
