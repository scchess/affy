...
\item Due to some new protocols \verb+?AffyBatch+ no longer will give you the help file. One needs to
to type \verb+class?AffyBatch+, \verb+help("AffyBatch-class")+ or \verb+?"AffyBatch-class"+.
...
The main difference between Version 1.1 and this version (1.2) is that in most cases the user no longer needs to manually download and install the appropriate CDF environment package. If the appropriate \verb+cdfenv+ is not installed it will, by default, be downloaded and installed automatically for the user. If a prepacked CDF environment for your particular chip type is not currently available you can create your own CDF environment. You may do this using the \Rpackage{makecdfenv} package which is available from the Bioconductor web site \url{www.bioconductor.org}. If you are using {\bf HGU95Av2} or {\bf HGU133A} chips, the
information is included in the \Rpackage{affydata} package. You can load this package instead of downloading further packages. See Section \ref{sec:probesloc} for information on how the environments work. 
...
%% LG: Depends on point of view. It is the case for people doing image
%% analysis, not for us so far... 
...
R> eset <- justRMA(Data)
\end{Sinput}
This reads the data and performs the `RMA' way to preprocess them at the {\it C} level.
