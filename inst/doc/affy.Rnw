...
{\bf What's new?}
\begin{itemize}
\item \verb+Cel+ class no longer supported. Function \verb+read.celfile+ and other \verb+Cel+ related methods and functions removed.
\item Speed and memory improvement of \verb+ReadAffy+ and \verb+read.affybatch+.
\item Function \verb+read.probematrix+ added. It reads CEL files and returns a matrix of PM, MM, or both. 
\item Package no longer depends on \verb+affydata+ package. For this reasone some examples have been moved from this vignette to the \verb+affydata+ vignette.
\end{itemize}
...
\Rpackage{makecdfenv} package to create one yourself.  
...
containing part of the data from a affybatch.example experiment. The full data
is publicly available from Gene Logic
\url{http://qolotus02.genelogic.com/datasets.nsf/} and described in
\cite{iriz:etal:2003}. The help file obtained via \verb+?affybatch.example+
...
This will create the \verb+affybatch.example+ object of class \Robject{AffyBatch}.
...
names and the row names are the {\it affyID}, e.g. \verb+AB000114_at+ and \verb+AB000115_at+ together with the probe
...
<<fig=TRUE>>=
par(mfrow=c(1,1))
boxplot(affybatch.example,col=c(2,2,3,3))
@
...
For an etended example on normalization please refer to the vignette in the affydata package.
