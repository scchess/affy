...
Section \ref{whatsnew} described the main differences between version 1.0 and
version 2.0. Section \ref{sec:get.started} describes a quick way of getting started and  
...
locations to probe set membership. Section \ref{configure.options} describes how to change the default options on the package.
...
In version 2.0, we have implemented various of the procedures
described in the MAS 5.0 manual. Throughout the package we use
\verb+mas+ to refer to procedures described in the MAS 5.0 software.
...
particular probe set. Beginners need do not need to understand these
...
And download and install the cdf environment you need
from 
\url{http://www.bioconductor.org/data/cdfenvs/cdfenvs.html} (except if
you are using the HGU95Av2 or HGU133A chip or making your own).
...
\item Reading the data and create an expression, using RMA for example.
\begin{Sinput}
...
For other popular methods use \verb+expresso+ instead of \verb+rma+. For example for MAS 5.0 signal you can use
\begin{Sinput}
R> eset <- expresso(Data, normalize=FALSE, bg.method="mas", 
...
See Figure \ref{fig:widget.filechooser}.
...
it is hard coded in C it is quite fast and efficient. The function
\verb+express+ lets the user define their own functions and send them
as arguments.
 
The function {\bf \it expresso} performs the steps background correction, normalization,
...
now show this using an \verb+AffyBatch+ included in the package for
...
Figure~\ref{fig:widget.normalizechooser} presents the widgets for the
...
Options for the package can be configured (see Section~\ref{configure.options}).

\subsection{C implementation of RMA}

One of the quickest ways to compute expression using the \verb+affy+ package is to use the \verb+rma+ function. We have found that this method allows a user to compute the RMA expression measure in a matter of minutes for datasets that may have taken hours in previous versions of \verb+affy+. The function serves as an interface to a hard coded C implementation of the RMA method \cite{iriz:etal:2003}. Generally, the following would be sufficient to compute RMA expression measures:
\begin{Sinput}
R> Data <-  ReadAffy() ##read data in working directory
R> eset <- rma(Data)
\end{Sinput}
...
\begin{enumerate}
\item Probe specific correction of the PM probes using a model based on observed intensity being the sum of signal and noise
\item Normalization of corrected PM probes using quantile normalization \cite{bols:etal:2002}
\item Calculation of Expression measure using median polish
\end{enumerate}

The \verb+rma+ function is likely to be improved and extended in the future as the RMA method is fine-tuned.
...
file). The default is quantile normalization \citep{bols:etal:2002}). 
...
number of physical rows on the array + $y$. The \verb+ncol+ and
...
mapping. More on this in Section \ref{probesloc}.
...
and $y$
coordinates as one number ($x \times$ number of rows $+$ $y$).
...
\verb+cleancdfname(x@cdfName)+. Here 
are two examples.
...
Package-wide options can be configured. We will show how to configure
the options with a first example: 
the compression of files. If you are always compressing your CEL
files, you might find annoying to specify 
it each you read them. I can be specified once for all in the
options. 
<<>>=
opt <- getOption("BioC")
affy.opt <- opt$affy
...
The second example explains how the finding locations of probes
environment works.
<<>>=
opt <- getOption("BioC")
affy.opt <- opt$affy
