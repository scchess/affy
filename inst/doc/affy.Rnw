...
{\bf Note:} If you use this package please cite \cite{iriz:gaut:cope:2003}.
...
\item slot 'preprocessing' of the MIAME attribute used to store
  ...
  \item tuning of the MAS5.0 methods implemented (bgcorrect.mas, ...). \footnote{A comparison between official MAS5.0 and the {\tt affy} implementation can be found at \url{http://stat-www.berkeley.edu/~bolstad/MAS5diff/Mas5difference.html}.}
\item The function \verb+mas5+ now available as a wrapper for \verb+expresso+ and \verb+affy.scalevalue.exprSet+.
\item method \verb+plot.ProbeSet+, an alternative to barplot, to plot probe
  level information.
\item parameter 'scale' in the method barplot for ProbeSet. All the
  barplots are scaled to eachothers. 
\item New functions \verb+xy2indices+ and \verb+indices2xy+ to shuttle from
...
\begin{itemize}
\item Some of the large example datasets have been moved to a a new package \verb+affydata+.
...
\item The default background on the \verb+rma+ function has been changed. Now 
  the results from \verb+rma+ and \verb+expresso+ should agree completely.
\item  The function \verb+express+ is deprecated. It still functions normally 
  but gives warning mesage. It will be removed in a future release.
  The function \verb+expresso+ should be used as a replacement.
\item bug in the parser fixed (infinite loop reported with 
...
\item barplot.ProbeSet scales plots to each other by default.
\end{itemize}
...
affecting you, or even better, on how you think our code could be improved are welcome} .
 Throughout the package we use \verb+mas+ to denote the procedures described in the MAS 5.0 software.
...
library(affy)
@
This release of the \verb+affy+ pacakge will automatically download the appropriate cdf environment when you require it. However, if you wish you may download and install the cdf environment you need from \url{http://www.bioconductor.org/data/cdfenvs/cdfenvs.html} manually. If there is no cdf environment currently built for your particular chip and you have access to the CDF file then you may use the \verb+makecdfenv+ pacakge to create one yourself.
...
R> eset <- justRMA(Data)
\end{Sinput}
Notice one does not need to call \verb+ReadAffy+ and thus probe level data is never stored.
\end{enumerate}

The \verb+rma+ function was written in C for speed and
efficiency. It uses the expression measure described in \cite{iriz:etal:2003}.

For other popular methods use \verb+expresso+ instead of \verb+rma+. For example for our version of MAS 5.0 signal you can use
\begin{Sinput}
R> eset <- expresso(Data, normalize=FALSE, bgcorrect.method="mas",
                     pmcorrect.method="mas",summary.method="mas")
\end{Sinput}

For MAS 5.0 one can simply use 
\begin{Sinput}
R> eset <- mas5(Data)
\end{Sinput}
which will aslo normalize the expression values. The normalization can be turned off through the \verb+normalize+ argument. 
...
In all the above examples, the variable \verb+eset+ is an object of class \verb+exprSet+
...
The function \verb+ReadAffy+ is quite flexible. It
...
The function \verb+ReadAffy+ is a wrapper for the functions
...
As mentioned the function \verb+expresso+ provides many options. For example,
\begin{Sinput}
...
For users of Windows, who wish to use Excel, the convenient function \verb+exprs2excel+ will write out a comma delimted file of expression values. You should be able to open this file by double clicking in Windows (use a {\tt .csv} file extension).
...
%The function \verb+express+ lets the user define their own functions and send them as arguments.


\subsubsection{expresso}

The function \verb+expresso+ performs the steps background correction, normalization,
probe specific correction, and summary value computation.
We now show this using an \verb+AffyBatch+ included in the package for
examples. The \verb+data(affybatch.example)+ is used to load these data.
...
@
Here use \verb+mas+ to refer to the methods described in the
...
To obtain MAS 5.0 use\verb+mas5+ which wraps \verb+expresso+ and \verb+affy.scalevalue.exprSet+
...
%measures. The function \verb+affy.scalevalue.exprSet+ performs a
%normalization similar to that described in the MAS 5.0 manual.

A detailed comparison between the MAS 5.0 values that are computed by \verb+affy+ and by Affymetrix can be found at \url{http://stat-www.berkeley.edu/~bolstad/MAS5diff/Mas5difference.html}.
...
%%Or you can use \verb+express+ to use your own implementation.

\subsubsection{C implementation of RMA}
One of the quickest ways to compute expression using the \verb+affy+
package is to use the \verb+rma+ function. We have found that this
...
of \verb+affy+. The function serves as an interface to a hard coded C
implementation of the RMA method \cite{iriz:etal:2003}. Generally, the
...
\end{Sinput}
%@
Currently the \verb+rma+ function implements RMA in the following manner
\begin{enumerate}
...
\item Normalization of corrected PM probes using quantile
  normalization \cite{bols:etal:2003}
\item Calculation of Expression measure using median polish.
\end{enumerate}

The \verb+rma+ function is likely to be improved and extended in the
...
data.  The plotting functions \verb+boxplot+ and \verb+hist+ have methods
for \verb+AffyBatch+ objects.  Each of these functions presents
...
mva.pairs (applied to \verb+pm(afbatch)+), offers pairwise
...
\verb+print+ (or \verb+show+) will display summary information.
...
pm(Dilution,gn[1000])
@
The method \verb+geneNames+ extracts the unique {\it
...
The methods \verb+boxplot+, \verb+hist+, and
\verb+image+
are useful for quality control.
Figure \ref{f3} shows histograms of $PM$ intensities
...
The functions \verb+AffyRNAdeg+, \verb+summaryAffyRNAdeg+, and
\verb+plotAffyRNAdeg+  aid in assessment of RNA quality.  Individual probes in
...
function \verb+plotAffyRNAdeg+ produces a side-by-side plots of these
means, making it easy to notice any $5'$ to $3'$ trend.  The function
\verb+summaryAffyRNAdeg+ produces a single summary statistic for each
...
Finally a plot can be created using \verb+plotAffyRNAdeg+, see Figure \ref{f4.3}.
...
cel with physical coordinates $(x,y)$ will be in row $i=x\times$
\verb+nrow+ $y + 1$. The \verb+ncol+ and
...
\verb+ncol+$\times$\verb+nrow+. For consistency with the previous
...
We store this mapping information in {\it R} environments. For each
...
The call to \verb+data+ loads an \verb+AffyBatch+ and also an
environment that corresponds to a small corner of the HG6800 chip,
which we created.
<<>>=
...
environment, then in a packages named \verb+cleancdfname(x@cdfName)+,
and finally in
