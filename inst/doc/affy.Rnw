...
Cel} files (the function {\Rfunction{getwd}, \Rfunction{setwd}, and
\Rfunction{list.celfiles}} are useful for this).
...
\Rfunction{mva.pairs} (applied to \verb+pm(afbatch)+), offers pairwise
