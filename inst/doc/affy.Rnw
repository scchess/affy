...
suite summarizes the probe set intensities to form one {\it
expression measure} for each gene. The expression measure is the data
available for analysis. However, as pointed out by
...
expression measures similar to MAS 4.0's AvDiff \citep{affy4}, MAS
5.0's signal \citep{affy5}, DChip's MBEI \citep{li:wong:2001a}, and
RMA \citep{iriz:etal:2003}. 
...
routines. Section \ref{classes} describes the different classes in the
package. \ref{sec:probesloc} describes our strategy to map probe
locations to probe set membership. Section \ref{configure.options}
describes how to change the package's default options. Section
\ref{whatwasnew} describes earlier changes. 
...
class \Robject{exprSet} 
described in the Biobase vignette. Many of the packages in
Bioconductor work on objects of this class. See the
\Rpackage{genefilter} and 
\Rpackage{geneplotter} packages for some examples.
...
\end{Sinput}
or if on Microsfot Windows and interested in reading your data into
excel 
\begin{Sinput}
...
phenotype, and MIAME information.
...
options. For example, 
\begin{Sinput}
R> eset <- expresso(affybatch, normalize.method="qspline", bg.method="rma",pmcorrect.method="pmonly",summary.method="liwong")
\end{Sinput}
...
For users of Microsoft Windows, who wish to use Excel, the convenient
function 
\Rfunction{exprs2excel} will write out a comma delimted file of
expression values. You should be able to open this file by double
clicking in Windows (use a {\tt .csv} file extension). 
...
We now show this using an \Robject{AffyBatch} included in the package
for 
examples. The command \verb+data(affybatch.example)+ is used to load
these data. 
...
To obtain MAS 5.0 presnce calls you can use the \verb+mas5calls+ method.

<<>>=
Calls <- mas5calls(affybatch.example)
@ 
This returns an \verb+exprSet+ with P/M/A calls in the \verb+exprs+
slot and the wilcoxon p-values in the \verb+se.exprs+ slot.
...
data.  The plotting functions \Rfunction{boxplot} and \Rfunction{hist}
have methods 
...
\Rfunction{MAplot} (applied, for example, to \verb+pm(affybatch.example)+), 
offers pairwise 
graphical comparison of intensity data. The option \verb+pairs+ permits you to chose between all pairwise comparisons (when \verb+TRUE+) or compared to a reference array (the default). 
These plots can be particularly
useful in diagnosing problems in replicate sets of arrays. The function argument \verb+plot.method+ can be used to create a MAplot using a smoothScatter, rather than the default method which is to draw every point. To use this option you will need to the \Rpackage{geneplotter} library installed.
...
versions the accessor method \verb+intensity+ exists for obtaining the
\verb+exprs+ slot.
...
interested in redefining the \verb+AB000114_at+ and \verb+AB000115_at+ probe
...
\url{http://www.bioconductor.org/data/metaData.html}, that contains
...
\item Most normalization routines for AffyBatches can now be called with the parameter \verb+type+ which specifies whether the normalization should be applied as a PM-only, MM-only, both PM and MM together or PM and MM separately.
