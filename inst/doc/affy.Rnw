...
from \url{http://www.bioconductor.org/data/cdfenvs/cdfenvs.html} (except if
...
\subsection{Li and Wong's MBEI (dchip)}
...
\begin{itemize}
\item {\bf affy: Custom Processing Methods (HowTo)}: A description of how to use custom preprocessing methods with the package.
