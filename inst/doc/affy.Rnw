...
this version. Section \ref{sec:get.started} describes a quick way of getting started and  
...
\verb+mas+ to denote the procedures described in the MAS 5.0 software.
...
\begin{Sinput}
%<<>>=
R> eset <- expresso(affybatch.example, normalize.method="invarianteset", 
                 bg.correct=FALSE,
...
\item Normalization of corrected PM probes using quantile
  normalization \cite{bols:etal:2002} 
