...
\ref{f2}. In order to 
do this, the \verb+widget+ argument must be true like this:
\begin{Sinput}
...
$i$th entry of the name \verb+name+ slot (accessed with {\tt probeName}).
...
arrays. Standard error estimates are also provided in the \verb+se+ slot.
...
section Section \ref{s3.2} and reading the 
...
carefully crafted in the function \verb+express+, we also encourage
...
In the following example, we look for the physical locations on a chip
...
in data exploration.  The top panels show the effect of an air
bubble. In the bottom panels we see ``cloudy'' areas which were
caused by a leak in an assembly joint around the devices for injecting
solutions onto the chip. Both are shown with raw (left panels) and
log-transformed values (right panels). In our experience, the log
...
known concentrations. The concentrations (pM) are also available
