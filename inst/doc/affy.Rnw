...
\item Speedn and memory improvement of \verb+ReadAffy+ and \verb+read.affybatch+.
\item Package no longer depends on \verb+affydata+ package. For this reasone many examples have been moved from this vignette to the \verb+affydata+ vignette.
\end{itemize}
