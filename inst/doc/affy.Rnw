...
1.2 and 
this version (1.3). Section \ref{sec:get.started} describes a quick
...
\section{Changes in versions 1.4.x}
\label{whatsnew}
To be done (check the file NEWS of the package in the meanwhile\ldots)
...
to enter phenotype or covariate information.
...
\Rfunction{mva.pairs} (applied, for example, to \verb+pm(affybatch.example)+), 
offers pairwise 
...
\section{Changes in Version 1.3.x}
