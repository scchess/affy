...
\item Package no longer depends on \verb+affydata+ package. For this
  reasone some examples have been moved from this vignette to the
  \verb+affydata+ vignette. 

\end{itemize}

There are some minor differences in what you can do but little
functionality has disappeared. Memorey efficiency and speed have improved.
...
library(affy)
@
This release of the \Rpackage{affy} pacakge will automatically
