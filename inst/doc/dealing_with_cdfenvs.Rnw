...
options for the package \Rpackage{affy} are outlined. To some extent,
what is developed here could\footnote{Currently this would require
...
to the building of packages with {\it cdfenvs}. The simpliest way
to proceed is to have the package needed for the analysis installed.
A number of {\it cdfenv} package for {\it Affymetrix} chips are available for download on
 the {\it Bioconductor} data packages repository. This release of the
\Rpackage{affy} package has an option to allow automatic downloading
and install of a {\it cdfenv} that would be found missing during an analysis.
...
The option \Rfunarg{autoload} is only relevant where \Rfunarg{where}
...
The autoload mechanism can be perceived as a security breach. It is the
