...
{\it probeset id} with indexes for the corresponding probes. These
indexes are used to subset a matrix storing probe level
...
A number of {\it cdfenv} package for {\it Affymetrix} chips are
...
will be made for you (and everybody else).  

If you do not care about mapping {\it probeset id}s with indexes 
and only want to deal with intensities, you can
use the following work around:

\begin{Sinput}
R> x <- ReadAffy() ##read-in your cel-file info
R> assign(x@cdfName, new.env() )
\end{Sinput}

The affy package will then find the empty environment defined by
\verb+x@cdfName+ and will not try to find a package. But beware: if
you do this and then install the appropriate package, you will
continue to use the empty environment until it is removed.
